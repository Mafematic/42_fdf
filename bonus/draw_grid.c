...
static void	draw_segment(t_image *i, t_window *w, t_pixel *start, t_pixel *end)
{
	put_pixel_to_image(i, w, start->w_x, start->w_y);
	put_pixel_to_image(i, w, end->w_x, end->w_y);
	draw_line(i, w, start, end);
}
...
void	draw_grid(t_pixel ***grid, t_window *w, t_prog_data *data)
{
	...
	while (row < data->rows)
	{
		...
		while (col < data->cols)
		{
			i.color = get_color(grid[row][col]->z);
			if (col < data->cols - 1)
				draw_segment(&i, w, grid[row][col], grid[row][col + 1]);
			if (row < data->rows - 1)
				draw_segment(&i, w, grid[row][col], grid[row + 1][col]);
			col++;
		}
		row++;
	}
